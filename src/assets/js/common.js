...
function turnstileSuccessCallback() {
    ...
    captchaState = true;
    changeButtonState(document.validateButton, Object.values(document.validateInputs), true);
}
...
/**
 * @brief Permet d'activer ou de désactiver un bouton en fonction de l'état des éléments nécessaires
 * @param button {HTMLElement} Bouton à activer ou désactiver
 * @param neededActive {Array<boolean>} Tableau contenant l'état des éléments nécessaires
 * @param needCaptcha {boolean} Indique si le Captcha est requis pour activer le bouton
 * @return void
 */
function changeButtonState(button, neededActive, needCaptcha = false) {
    if (needCaptcha) {
        neededActive.push(captchaState);
    }

    if (neededActive.every((element) => element === true)) {
